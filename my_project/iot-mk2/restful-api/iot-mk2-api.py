...
parser.add_argument('value', type=float, required=True, help='need measurement')


class IOT_MK2(Resource):
    def __init__(self):
    ...
    def get(self):
        ...
        #return self.iot_msg
        return "hello GET client"

    def post(self):
        post_result = "Hello POST user"
    ...
    try:
        ...
        for current_dbname in database:
            ...
            if dbname in (item[u'name']):
                print("InfluxDB: DB %s is exist, switch database to %s") % (dbname, dbname)
                db_exist = True
...
api.add_resource(IOT_MK2, '/v1/iot-mk2')
